...
public class AccountServiceImpl implements AccountService{
	...
	public <S extends Account> S save(S entity) {
		...
		Optional<Account> acExist = findById(entity.getUsername());
		
		if(acExist.isEmpty()) {
			if(StringUtils.isEmpty(entity.getPassword())) {
				entity.setPassword(acExist.get().getPassword());
			}else {
	...
	public Account login(String username,String password) {
		Optional<Account> optExists = findById(username);
		
		if(optExists.isPresent() && bCryptPasswordEncoder.matches(password, optExists.get().getPassword())) {
			optExists.get().setPassword("");
			return optExists.get();
		}
		return null;
	}
